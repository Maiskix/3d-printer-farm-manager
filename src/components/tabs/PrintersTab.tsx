            ...
            принтеры.map((p) => (
              <div key={p.id} className="animate-fade-in group rounded-2xl bg-card p-4 shadow-sm">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button onClick={() => управлять(p.id, 'печать')} className="flex items-center justify-center gap-1 rounded-lg bg-farm-blue py-2 text-[11px] font-medium uppercase text-white transition-transform active:scale-95">
                    <Icon name="Play" size={12} /> Старт
                  </button>
                  <button onClick={() => управлять(p.id, 'пауза')} className="flex items-center justify-center gap-1 rounded-lg bg-farm-teal py-2 text-[11px] font-medium uppercase text-white transition-transform active:scale-95">
                    <Icon name="Pause" size={12} /> Пауза
                  </button>
                  <button onClick={() => управлять(p.id, 'простой')} className="flex items-center justify-center gap-1 rounded-lg bg-muted py-2 text-[11px] font-medium uppercase text-foreground transition-transform active:scale-95">
